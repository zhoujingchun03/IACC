...
from utils.common_utils import MetricRecorder, EMA
...
def train(
    ...
    for current_epoch in range(start_epoch, max_epochs + 1):
        train_return = train_one_epoch(hparams, model, scaler, optimizer, scheduler, ema, train_loader)
        logger.log_multi_scaler(train_return, current_epoch)
        scheduler.step(current_epoch)
        # valid
        valid_result = None
        if current_epoch % hparams['train']['valid_frequency'] == 0:
            valid_result = valid(hparams, model, ema, valid_loader)
